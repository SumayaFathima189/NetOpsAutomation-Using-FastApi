...
# Dependency to retrieve the token
def get_token(token: str = Depends(get_Token)):
    # Code to obtain the token from FDM
...
@app.get("/network objects", tags=["NETWORK OBJECTS"])
def get_network(token: str = Depends(get_token)):
...
def create_network_object(network_data: dict, token: str = Depends(get_token)):
    url = "https://10.10.20.65/api/fdm/latest/object/networks"
    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {token}"
...
def delete_network_object(object_id: str, token: str = Depends(get_token)):
...
def update_network_object(object_id: str, network_object_data: dict, token: str = Depends(get_token)):
    url = f"https://10.10.20.65/api/fdm/latest/object/networks/{object_id}"
    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {token}"
...
def get_access_policies(token: str = Depends(get_token)):
    ...
    parentId=id[0].get('id')
    return parentId


@app.get("/get_access_rules", tags=["ACCESS POLICY"])
def get_access_rules(policy_id: str, token: str = Depends(get_token)):
...
def create_access_rule(policy_id: str, access_rule_data: dict, token: str = Depends(get_token)):
    url = f"https://10.10.20.65/api/fdm/latest/policy/accesspolicies/{policy_id}/accessrules"
    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {token}"
...
def delete_access_rule(policy_id: str, object_id: str, token: str = Depends(get_token)):
...
def update_access_rule(parent_id: str, object_id: str, access_rule_data: dict, token: str = Depends(get_token)):
...
def get_port_objects(limit: int, token: str = Depends(get_token)):
...
def create_port_object(port_data: dict, token: str = Depends(get_token)):
    url = "https://10.10.20.65/api/fdm/latest/object/tcpports"
    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {token}"
...
def delete_port_object(port_id: str, token: str = Depends(get_token)):
...
def update_port_object(port_id: str,  port_object_data: dict, token: str = Depends(get_token)):
    url = f"https://10.10.20.65/api/fdm/latest/object/ports/{port_id}"
    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {token}"
...
@app.get("/nat_objectnatid",  tags=["AUTO NAT RULES"])
def get_nat_objectid(token: str = Depends(get_token)):
...
# Get AUT0 NAT rules
@app.get("/nat_objectnat", tags=["AUTO NAT RULES"])
def get_nat_rules(parent_id: str, token: str = Depends(get_token)):
...
# Post AUTO NAT rules
@app.post("/create_object_nat", tags=["AUTO NAT RULES"])
def create_nat_rule(parent_id: str, nat_rule_data: dict, token: str = Depends(get_token)):
    url = f"https://10.10.20.65/api/fdm/latest/policy/objectnatpolicies/{parent_id}/objectnatrules"
    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {token}"
...
# Delete AUTO NAT rules
@app.delete("/delete_nat_rule/{policy_id}/{rule_id}", tags=["AUTO NAT RULES"])
def delete_nat_rule(parent_id: str, object_id: str, token: str = Depends(get_token)):
...
# Update AUTO NAT rules
@app.put("/update_nat_rule/{rule_id}", tags=["AUTO NAT RULES"])
def update_nat_rule(parent_id: str, object_id: str, nat_rule_data: dict, token: str = Depends(get_token)):
    url = f"https://10.10.20.65/api/fdm/latest/policy/objectnatpolicies/{parent_id}/objectnatrules/{object_id}"
    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {token}"
